...
class MotionMapping():
    ...
    def update_vicon_marker(self, markers):
        ...
        self.hand_right_rot = np.matrix([[markers[4].x], [markers[4].y], [markers[4].z]])
        ...
        self.hand_left_rot = np.matrix([[markers[9].x], [markers[9].y], [markers[9].z]])
        ...
        # print(self.elbow_right_r, self.wrist_right_r1, self.wrist_right_r2, self.hand_right_r, self.index_right_r)
        #print("left_parameters:")
        # print(self.elbow_right_l, self.wrist_right_l1, self.wrist_right_l2, self.hand_right_l, self.index_right_l)
    ...
    def update_body_paramters(self):
        ...
        temp_length = np.linalg.norm(self.pelvis_right - self.knee_right) + np.linalg.norm(self.knee_right - self.heel_right)
    ...
    def gripper_mapping_right(self):
        dist = math.sqrt((self.index_right[0] - self.thumb_right[0]) ** 2 + (self.index_right[1] - self.thumb_right[1]) ** 2
    ...
    def detect_rotation(self): # TODO : Rotation based on height of palm of the hand, should add function to return 0 or 1 based on position and rotate accrodingly
    ...
    def ik_baxter_full(self, ee, approach, swivel):
        ...
        pw = ee - self.l4 * n # TODO : Change value here, remove self.l4
    ...
    def wrist_mapping(self, human_wrist):
        ...
        ratio = 1.2*(self.robot_arm_length / self.arm_length)
    ...
    def for_rot(self):
        if (self.shoulder_right[2]/1000 + 0.1 <= self.hand_right_rot[2]/1000) and (self.shoulder_left[2]/1000 + 0.1 <= self.hand_left_rot[2]/1000):
            a = 1
        else:
            a = 0
        return a, self.shoulder_right[2]/1000, self.hand_right_rot[2]/1000
