...
class CartesianPoseBaseController(TaskController):
    """Task: CartesianPose
    ...
    def taskName(self):
        return 'CartesianPoseBase'

    def start(self,args):
        print "Calling CartesianPoseBase start",args
        if 'limb' not in args:
	...
	if 'base_velocity' not in args:
        ...
        if limb == 'left' or limb == 0:
            ...
            return False
        if 'base_velocity' in args:
            base_v = args['base_velocity']
            if len(base_v) != 3:
                ...
                for i,v in enumerate(base_v):
                    if abs(v) < 1e-3:
                        base_v[i] = 0
                robot.base.moveVelocity(base_v[0],base_v[1],base_v[2])
        self._status='ok'
        if safe:
            robot.enableCollisionChecking(False)
        self._status = 'ok'
...
def make():
    return CartesianPoseBaseController()
