...
class KeyboardAutoTrackTaskGenerator(TaskGenerator):
    ...
    def __init__(self):
        ...
        self.z = None
        self.w = None
        self.rot_matrix_l = None
        self.rot_matrix_r = None
        self.tot_rot_matrix = []
        self.loc_l = [0.6647,0.18159,1.3294]
        self.loc_r = [0.6647,-0.2015,1.3294]
        ...
        self.vive_arm_orientation_l = rospy.Subscriber('/Right_Hand', TransformStamped, self.callback_arm_location_l, queue_size = 1)
        self.vive_arm_orientation_r = rospy.Subscriber('/Left_Hand', TransformStamped, self.callback_arm_location_r, queue_size = 1)
        self.vive_1 = rospy.Subscriber('/vive/controller_LHR_FF7FBBC0/joy', Joy, self.callback_vive_r, queue_size = 1)
        self.vive_2 = rospy.Subscriber('/vive/controller_LHR_FFFB7FC3/joy', Joy, self.callback_vive_l, queue_size = 1)
    ...
    def callback_arm_location_l(self,data):
        ...
        self.loc_l[0] = 1.276*data.transform.translation.x
        self.loc_l[1] = 1*data.transform.translation.y
        self.loc_l[2] = 1.109*data.transform.translation.z
        self.rot_matrix_l = quaternion_matrix([x,y,z,w])
        self.rot_matrix_l = self.rot_matrix_l[:3,:3].flatten()
        self.rot_matrix_l = self.rot_matrix_l.tolist()
    ...
    def callback_arm_location_r(self,data):
        ...
        z = data.transform.rotation.z
        w = data.transform.rotation.w
        self.loc_r[0] = 1.276*data.transform.translation.x
        self.loc_r[1] = 1*data.transform.translation.y
        self.loc_r[2] = 1.109*data.transform.translation.z
        self.rot_matrix_r = quaternion_matrix([x,y,z,w])
        self.rot_matrix_r = self.rot_matrix_r[:3,:3].flatten()
        self.rot_matrix_r = self.rot_matrix_r.tolist()
    ...
    def do_logic(self):
        ...
        if self.plugin.viveControl == Ture:
        	self.plugin.viveControl = False
        	self.tot_rot_matrix = self.rot_matrix_l + self.rot_matrix_r
            loc = self.loc_l + self.loc_r
            #self.tot_rot_matrix.extend(self.rot_matrix_l)
            #self.tot_rot_matrix.extend(self.rot_matrix_r)
            ...
            "limb":"both", "position":loc,
            "rotation" : self.tot_rot_matrix, 
            "speed":1,
